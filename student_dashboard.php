...
$sql = "SELECT status,acad_year from current_state";
$res = $conn->query($sql);                     
...
$query = "SELECT course_code FROM subject where sem='$sem' and dept_id='$dept_id' and acad_year='$acad_year'";
$res = $conn->query($query);                     
while($r=$res->fetch_assoc()){ 
	$course_code=$r['course_code'];
	if($status==0){
		$m = "SELECT * FROM response_midsem where  course_code='$course_code' and acad_year='$acad_year' and roll_no='$roll_no'";
		$n = $conn->query($m); 
		$noOfStudents=$n->num_rows;
	}
	else{
		$m = "SELECT * FROM response_endsem where  course_code='$course_code' and acad_year='$acad_year' and roll_no='$roll_no'";
		$n = $conn->query($m); 
		$noOfStudents=$n->num_rows;
	}
	if($noOfStudents==0){
...
if($_SESSION['class']=='LY' or $_SESSION['class']=='MTech Comp' or $_SESSION['class']=='MTech IT' or $_SESSION['class']=='MTech ETRX' or $_SESSION['class']=='MTech EXTC' or $_SESSION['class']=='MTech Mech'){
  $query = "SELECT electiveID,electiveBatchID FROM student where dept_id='$dept_id' and acad_year='$acad_year' and roll_no='$roll_no'";
  $res = $conn->query($query);                     
  while($r=$res->fetch_assoc()){ 
    $course_code=$r['electiveID'];
    if($status==0){
      $m = "SELECT * FROM response_midsem where  course_code='$course_code' and acad_year='$acad_year' and roll_no='$roll_no'";
      $n = $conn->query($m); 
      $noOfStudents=$n->num_rows;
 ...
 <!-- <h1 id="h1"> K.J SOMAIYA COLLEGE OF ENGINEERING</h1>
  ...
  <script type="text/javascript">
  	...
  	function nextCourse(){
  		...
  		document.getElementById(courses[count-1]).className="badge-success";
    ...
    <div id="main" style=" width: 80vw; margin-left: 5vw;">
     ...
     <script type="text/javascript">
      if(courses.length==0){
       ...
       parent.innerHTML="You have already submitted the feedback";
     }
   </script>
   <script type="text/javascript">
