...
$sql = "SELECT start_time,end_time from current_state where dept_id='$dept_id'";
$res = $conn->query($sql);                     
...
$sql_student = "SELECT flag FROM student WHERE roll_no = '$roll_no'";
...
if($flag==0):
	...
	$sem=$_SESSION["sem"];
	$dept_id=$_SESSION["dept_id"];
	$querycheck = mysqli_query($conn,"SELECT * FROM electives WHERE electiveID = '$course_code'");
	

	
	if(mysqli_num_rows($querycheck)>0){
		$query = "SELECT electiveName,f_id FROM electives where dept_id='$dept_id' and acad_year='$acad_year' and electiveID='$course_code'";
					...
					<div class="row">
						...
						<div class="panel panel-primary">
							...
							<?php if($q_id==$noOfQues):?>
								<ul class = "list-group" >
									<?php
									$sql2 = "SELECT option_no,`option` FROM options where course_type='$c' and q_id=$q_id";
...
<?php 
echo "<button style='width:200px; height:50px; font-size:20px;' class='btn btn-primary'  onclick='validateForm()' id='next'>SAVE AND NEXT</button>";
