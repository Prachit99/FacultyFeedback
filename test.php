...
<div class="canvas_div_pdf">
  ...
  if($sem_type=="Odd"){
    if($status==0)
      ...
      echo '<p style="text-align: center; font-size: 18px;"><strong>Even Semester (Mid Term) </strong></p>';
  }else{
    if($status==0)
      echo '<p style="text-align: center; font-size: 20px;"><strong>Odd Semester (End Term) </strong></p>';
    else
  ...
  $s2 = "SELECT course_code,class,sem,section_or_batch from courses_faculty where f_id='$f_id' and sem%2=1";
  $r2= $conn->query($s2);
  while($res2=$r2->fetch_assoc()):
    ...
    <?php echo "<br><p style=' text-align:center;'><b style='font-size: 18px;'>Faculty Evaluation: </b><strong style='color:#162252; font-size: 20px; '>".$avg."</strong></p>";
    if($status==0){
     ...
     while($comment=$res->fetch_assoc()){
      echo "<b>".$index.") ".$comment["comment"]."</b><br>";
      $index++;
    }
  }else{
...
while($res2=$r2->fetch_assoc()):
  ...
  $a="SELECT roll_no from student where sem='$sem' and (electiveID='$electiveID' or electiveBatchID='$electiveID')  and acad_year='$acad_year'";
  $b=$conn->query($a);
  ...
  echo "<b style='font-size: 18px;'>Elective Name: </b><strong style='color:#162252;font-size: 16px;'>".$electiveName."</strong>&nbsp;&nbsp;&nbsp;&nbsp;";
 ...
 <table style="border-radius: 5px;"><tr>
  ...
  while($row=$result->fetch_assoc()):
    ...
    <td style="width: 50%; text-align: center; border:0.5px solid #162252 ;padding: 5px; background-color: #f5f5f5 ">
      <?php echo "<br><b>".$question."</b><br>"; ?>
      <canvas id='<?php echo $cname. $electiveID.$q_id ?>' width="50%" height="180" ></canvas> 
    </td>
    <script>
      var my_canvas=document.getElementById(<?php echo json_encode($cname. $electiveID.$q_id)?>);
      var gctx=my_canvas.getContext("2d");
...
<?php echo "<br><p style=' text-align:center;'><b style='font-size: 18px;'>Faculty Evaluation: </b><strong style='color:#162252; font-size: 20px; '>".$avg."</strong></p><br><hr><br>";
if($status==0){
